...
import svgPaths from "./imports/svg-abiszimvds";
...
function User1({ onClick }: { onClick: () => void }) {
  return (
    <div 
      className="w-[clamp(280px,25vw,350px)] aspect-square rounded-[18px] shadow-[-2px_2px_4px_0px_rgba(98,98,98,0.2),2px_-2px_4px_0px_rgba(98,98,98,0.2),-2px_-2px_4px_0px_rgba(255,255,255,0.9),2px_2px_5px_0px_rgba(98,98,98,0.9)] relative cursor-pointer hover:scale-105 transition-transform" 
      data-name="User"
    ...
    <div className="bg-white min-h-screen flex flex-col items-center px-4" data-name="Netflix Party First Screen">
      ...
      <div className="flex-1 flex items-center justify-center">
        <div className="flex gap-[clamp(3rem,8vw,6rem)] items-center justify-center flex-wrap">
          <User1 onClick={() => onSelectUser('Fred')} />
          <User onClick={() => onSelectUser('Avalene')} />
        </div>
      </div>
    </div>
  );
...
function LoginButton() {
  return (
    <div className="shrink-0 size-6" data-name="Login Button">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 24 24">
        <g id="Login Button">
...
function PasswordSubmitBox() {
  ...
  return (
    <div className="flex gap-[5px] items-center justify-center w-full max-w-[312px]" data-name="Password submit box">
      ...
      {password.trim().length > 0 && <LoginButton />}
    </div>
  );
}

function PasswordScreen({ selectedUser, onBack }: { selectedUser: string; onBack: () => void }) {
  return (
...
export default function App() {
  ...
  if (selectedUser) {
    return <PasswordScreen selectedUser={selectedUser} onBack={handleBack} />;
  }

  return <ProfileSelectionScreen onSelectUser={handleSelectUser} />;
}